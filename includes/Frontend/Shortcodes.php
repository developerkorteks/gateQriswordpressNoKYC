...
final class Shortcodes {
	...
	public function render_form( array $atts = array() ): string {
		...
		ob_start();
		?>
		<form class="gateqris-payments-form" method="post" action="<?php echo esc_url( $this->public_rest_url( 'gateqris/v1/transactions' ) ); ?>" data-gateqris-rest-nonce="<?php echo esc_attr( wp_create_nonce( 'wp_rest' ) ); ?>">
			...
			<p>
				<label><?php esc_html_e( 'Nominal (Rp)', 'gateqris-payments' ); ?><br />
					<input type="number" min="1" name="amount" value="<?php echo esc_attr( (string) $atts['amount'] ); ?>" required />
				</label>
			</p>
			<?php if ( 'user' !== $target ) : ?>
				<p>
					<label><?php esc_html_e( 'Nama', 'gateqris-payments' ); ?><br />
				...
				<p>
					<label><?php esc_html_e( 'Catatan', 'gateqris-payments' ); ?><br />
						<input type="text" name="reference" value="" />
					</label>
				</p>
			<?php endif; ?>
			<p><button type="submit"><?php echo 'user' === $target ? esc_html__( 'Isi Saldo', 'gateqris-payments' ) : esc_html__( 'Lanjut Bayar QRIS', 'gateqris-payments' ); ?></button></p>
		</form>
		<script>
		document.addEventListener('submit', async function (event) {
			...
			// Send the REST cookie nonce so WordPress authenticates the logged-in
			// user (required for user-wallet top-up); harmless for guests.
			const restNonce = form.getAttribute('data-gateqris-rest-nonce');
			if (restNonce) {
			...
			if (!response.ok) {
				...
				const stale = code.indexOf('nonce') !== -1 || code === 'gateqris_invalid_token';
				alert(stale
					? 'Sesi kedaluwarsa. Muat ulang halaman ini (refresh) lalu coba lagi.'
					: ((data && data.message) || 'Permintaan gagal. Silakan coba lagi.'));
				return;
			}
	...
	public function render_status( ?string $uuid = null, ?string $token = null ): string {
		...
		.gateqris-back{display:inline-block;margin-top:18px;text-decoration:none;background:var(--gq-text);color:var(--gq-bg);border-radius:999px;padding:11px 24px;font-family:var(--gq-font-head);font-size:14px;font-weight:500;transition:background .2s ease}
		.gateqris-back:hover{background:var(--gq-accent)}
		</style>
		<div class="gateqris-pay" data-gateqris-redirect="<?php echo esc_url( $success_redirect ); ?>">
			...
			<div class="gateqris-success" data-gateqris-success hidden>
				...
				<a class="gateqris-back" data-gateqris-back href="/" hidden><?php esc_html_e( 'Kembali', 'gateqris-payments' ); ?></a>
			</div>
		</div>
		...
		(function () {
			...
			(function pollStatus() {
				...
				var statusEndpoint = new URL(<?php echo wp_json_encode( $this->public_rest_url( 'gateqris/v1/transactions/' . rawurlencode( $uuid ) . '/status' ) ); ?>, window.location.origin);
				statusEndpoint.searchParams.set('access_token', <?php echo wp_json_encode( $token ); ?>);
				var refreshEndpoint = new URL(<?php echo wp_json_encode( $this->public_rest_url( 'gateqris/v1/transactions/' . rawurlencode( $uuid ) . '/refresh' ) ); ?>, window.location.origin);
				...
				var showSuccess = function () {
					...
					if (redirectUrl) {
						...
						var back = root.querySelector('[data-gateqris-back]');
	...
	private function public_rest_url( string $path ): string {
		// Same-origin (relative) URL for browser fetches, so the auth cookie and REST
		// nonce always match the page's origin — even when Public Base URL points to a
		// tunnel domain (which is only meant for server-to-server webhooks/hosted links).
		$relative = wp_make_link_relative( rest_url( $path ) );
		return '' !== $relative ? $relative : rest_url( $path );
	}
